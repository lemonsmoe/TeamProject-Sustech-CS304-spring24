...
class Scheduler:
    ...
    def __init__(self, name, course_keywords, bad_keywords, student_type='本', excel_path="./Excel/ClassTable.xlsx"):
        ...
        self.find_course(method='sqlite')
    ...
    def find_course(self, method='excel'):
        start = time.time()
        ...
        if method == 'excel':
            ...
            con, cur = connect_db(db_type='sqlite')
        ...
        for keyword in self.course_keywords:
            ...
            if method == 'excel':
                ...
                cur.execute(f"select * from Course2024Spring where col0 like '%{keyword}%' or col2 like '%{keyword}%' or col3 like '%{keyword}%';")
                for row in cur.fetchall():
                    if any([(badword in row[0] or badword in row[2]) for badword in self.bad_keywords]):
                        continue
                    if row[1] not in self.student_type:
                        continue
                    course = Course(row)
                    self.concrete_course[keyword].append(course)
    ...
    def get_concrete_course_info(self) -> dict:
        ...
        for keyword in self.concrete_course:
            ...
            for course in self.concrete_course[keyword]:
                all_info = course.all_info
                course_info[keyword].append(all_info[0])
        ...
        for keyword in self.concrete_course:
            ...
            for course in self.concrete_course[keyword]:
                ...
                if any([course in c for c in self.schedule_scheme]):
                    all_info = course.all_info[0]
                    code = course.all_info[2]
